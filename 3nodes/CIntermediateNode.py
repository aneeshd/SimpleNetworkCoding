...
while 1:
	...
	if not data:
		...
		print "\nReceived message from source node A is '", data,"'"
		dataA = data
		ibinarylist1 = StringToBinaryList(dataA)
		print 'Data received'
		print ibinarylist1
		print ''
		...
		# Receive messages
		while 1:
			data2, addr2 = UDPSock2.recvfrom(buf)
			if not data2:
				print "Client has exited!"
				break
			else:
				print "\nReceived message from source node B is '", data2,"'"
				dataB = data2
		...
		aaa = BackToString(listOfCodedData)
		print 'binary'
		binary = StringToBinaryList(aaa) 

		bb = ConcatListElements(aaa)
		print bb

		s = socket(AF_INET, SOCK_DGRAM)
		s.setsockopt(SOL_SOCKET, SO_BROADCAST,1)
		s.sendto(bb, ('<broadcast>' ,8881))
		s.close()		
